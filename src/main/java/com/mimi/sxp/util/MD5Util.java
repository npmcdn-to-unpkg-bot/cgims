...
public class MD5Util {
	...
	public static String encode(String salt,String rawPass) {
		String result = null;
		try {
			result = MD5(MD5(mergePasswordAndSalt(salt,
					rawPass)));
		} catch (Exception ex) {
		}
		return result;
	...
	public static boolean isPasswordValid(String salt,String encPass, String rawPass) {
		if(StringUtils.isBlank(encPass) || StringUtils.isBlank(rawPass)){
			return false;
		}
		return encPass.equals(encode(salt,rawPass));
	}
	...
	private static String mergePasswordAndSalt(String salt,String password) {
		...
		if (StringUtils.isBlank(salt)) {
			...
			return password + "mimi" + salt.toString();
		}
	}
	...
	public static void main(String[] args) {
		...
		String pwd = "admin";
		pwd = MD5(pwd);
		String encode = encode(salt,pwd);
		System.out.println(encode);
		boolean passwordValid = isPasswordValid(salt,
				"1bd98ed329aebc7b2f89424b5a38926e", pwd);
		System.out.println(passwordValid);

//		PasswordEncoder encoderSha = new PasswordEncoder(salt, "SHA");
		String pass2 = encode(salt,pwd);
		System.out.println(pass2);
		boolean passwordValid2 = isPasswordValid(salt,
				"1bd98ed329aebc7b2f89424b5a38926e", pwd);
