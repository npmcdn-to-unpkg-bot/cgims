	...
	<body>
						...
						<div class="box-cnt">
							<div class="datatable" id="repList">
								...
								<table class="datatable-table">
									<thead>
										<c:if test="${fn:contains(loginedUserPermissions,'mi:realEstateProject:delete') }">
										<th class="checkarea">
								...
								<div class="datatable-toolbar disabled">
									<div class="toolbar">
										...
										<c:if test="${fn:contains(loginedUserPermissions,'mi:realEstateProject:add') }">
										<a class="btn btn-primary" href="${ctx}/mi/realEstateProject/add">新增</a>
										</c:if>
									</div>
								</div>
		...
		<script type="text/x-handlebars" id = "rep-template">
			...
			<tr>
				...
				<td>
					<c:if test="${fn:contains(loginedUserPermissions,'mi:realEstateProject:view') }">
					<a class="btn btn-info" href="${ctx}/mi/realEstateProject/view/{{id}}">
						<i class="icon-zoom-in "></i>                                            
					</a>
					</c:if>
					<c:if test="${fn:contains(loginedUserPermissions,'mi:realEstateProject:update') }">
					<a class="btn btn-info" href="${ctx}/mi/realEstateProject/update/{{id}}">
						<i class="icon-edit "></i>                                            
					</a>
					</c:if>
					<c:if test="${fn:contains(loginedUserPermissions,'mi:realEstateProject:delete') }">
		...
		var deling = false;
	  	...
	  	function delRep(e,repIds){	  		
			if(!window.confirm("确认删除?")){
	  		...
	  		deling = true;
	  		var url = "${ctx}/mi/realEstateProject/delete/batch/json";
	  		$.ajax({
	  			type:"post",
	  			data:{"realEstateProjectIds":repIds},
	  			url:url,
	  			async:true,
	  			dataType:"json",
	  	...
	  	var page = new Page({
	  			...
	  			url:"${ctx}/mi/realEstateProject/search/page/json/",
	  			data:{"pageSize":10}
	  	});
	  	page.init();
