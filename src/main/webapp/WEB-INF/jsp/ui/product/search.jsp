...
<html>
<head>
<c:set var="headKeywords" value="商品"/>
<c:set var="headTitle" value="商品"/>
		...
		<c:choose>
			<c:when test="${total!=0}">
				<div id="drag" class="cenBtn">
					<a class="draginner" href="javascript:void(0);">查看更多商品</a>
				</div>
			</c:when>
			<c:otherwise>
				<div class="searchNo">
			    	<p class="f14">暂未搜索到符合条件的商品。</p>
				</div>
			</c:otherwise>
		</c:choose>
...
var curp = 1;
	...
	function load(){
		...
		var url = "${ctx}/product/search/json/{searchKeyword}-{targetPage}-{pageSize}";
			...
			success: function(json){
				if(json.success){
					for(var i=0;i<json.results.length;i++){
						...
						var str = ' <li ><a href="${ctx}/product/view/{id}" > <div class="bord"> <div class="lt"> <img alt="" src="{preViewUrl}"> </div> <div class="rt"> <h3>{name}</h3> <p>{introduction}</p> </div> </div> </a></li>';
	...
	function searchAction(){
		var url = "${ctx}/product/search/-{searchKeyword}-";
		url = bindSearchData(url);
// 		top.location.href = url;
		top.location = "${ctx}/product/search/-"+$("#searchKeyword").val()+"-";
	}
