...
<html>
<head>
<c:set var="headKeywords" value="商铺"/>
<c:set var="headTitle" value="商铺"/>
		...
		<c:choose>
			<c:when test="${total!=0}">
				<div id="drag" class="cenBtn">
					<a class="draginner" href="javascript:void(0);">查看更多商铺</a>
				</div>
			</c:when>
			<c:otherwise>
				<div class="searchNo">
			    	<p class="f14">暂未搜索到符合条件的商铺。</p>
				</div>
			</c:otherwise>
		</c:choose>
...
var curp = 1;
	...
	function load(){
		...
		var url = "${ctx}/shop/search/json/{searchKeyword}-{targetPage}-{pageSize}";
			...
			success: function(json){
				if(json.success){
					for(var i=0;i<json.results.length;i++){
						...
						var str = ' <li ><a href="${ctx}/shop/view/{id}" > <div class="bord"> <div class="lt"> <img alt="" src="{preViewUrl}"> </div> <div class="rt"> <h3>{name}</h3> <p>{introduction}</p> </div> </div> </a></li>';
						str = str.replace(/{id}/g,result.id);
						str = str.replace(/{preViewUrl}/g,result.preViewUrl);
						str = str.replace(/{name}/g,result.name);
						str = str.replace(/{introduction}/g,result.introduction);
						$('#itemList').append(str);
					}
				}else if(json.msg){
					alert(json.msg);
	...
	function searchAction(){
		var url = "${ctx}/shop/search/-{searchKeyword}-";
		url = bindSearchData(url);
// 		top.location.href = url;
		top.location = "${ctx}/shop/search/-"+$("#searchKeyword").val()+"-";
	}
